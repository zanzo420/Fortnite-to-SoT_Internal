...
void InitializeSDK()
{
	...
	MmBase = (uintptr_t)info.lpBaseOfDll;
	MmSize = info.SizeOfImage;

	auto refGobjs = FindPattern(E("48 8B D1 48 8D 0D ? ? ? ? 48 83 C4 38 E9")) + 3;
	auto refGnames = FindPattern(E("48 8B 05 ? ? ? ? 48 85 C0 75 55 B9 ? ? ? ?"));
	//G::GetBoneMatrix = (tGetBoneMatrix)FindPattern(E("40 53 ? ? 41 ? 48 81 EC ? ? ? ? 45 33 ? 49 63 ? 44 89 B4 24 ? ? ? ? 48 8B DA F6 81 ? ? ? ? ?"));

	auto rel = *(int32_t*)(refGobjs + 3) + 7;
	auto gobjs = refGobjs + rel;
	UObject::GObjects = (FUObjectArray*)gobjs;

	rel = *(int32_t*)(refGnames + 3) + 7;
	auto gnames = refGnames + rel;
	FName::GNames = *(TNameEntryArray**)gnames;
}
using namespace SDK;
...
void InitializeSDK()
{
	auto refGobjs = FindPattern("89 1D ? ? ? ? 48 8B DF 48 C1 E3 04 33 D2") + 3;
	auto refGnames = FindPattern("48 8B 3D ? ? ? ? 48 85 FF 75 ? B9 ? ? ? ? E8 ? ? ? ? 48 8B F8 48 89 44");
	//auto GetBoneMatrix = FindPattern("40 53 ? ? 41 ? 48 81 EC ? ? ? ? 45 33 ? 49 63 ? 44 89 B4 24 ? ? ? ? 48 8B DA F6 81 ? ? ? ? ?");

	auto gobjs = refGobjs + *(int32_t*)(refGobjs + 3) + 7;
	UObject::GObjects = (FUObjectArray*)gobjs;

	auto gnames = refGnames + *(int32_t*)(refGnames + 3) + 7;;
	FName::GNames = *(TNameEntryArray**)gnames;
}
